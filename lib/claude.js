...
const ANALYZE_SCHEMA = {
  ...
  properties: {
    ...
    templateReutilisable: { type: 'string', description: 'LA formule abstraite répétable, COURTE : 1 phrase (~15-25 mots max). PAS de déroulé étape par étape, PAS de crochets. Ne cite JAMAIS le nom d\'un outil concurrent (dis "un outil" ou "Creatikk").' },
...
PRÉFÈRE FUSIONNER : en cas de doute, réponds match=true plutôt que de créer un quasi-doublon. Ne crée un NOUVEAU template QUE si la mécanique de fond est VRAIMENT différente. Si le nom que tu donnerais ressemble à celui d'un template existant, alors c'est un match (match=true). Réponds en français.`;
...
const CLASSIFY_SCHEMA = {
  ...
  properties: {
    ...
    formule: { type: 'string', description: 'la formule abstraite réutilisable, COURTE : 1 phrase (~15-25 mots). Pas de déroulé à crochets. Jamais le nom d\'un concurrent.' },
...
const REFINE_SCHEMA = {
  ...
  properties: {
    ...
    formule: { type: 'string', description: 'la formule commune la plus précise, tirée de TOUTES les vidéos, mais COURTE : 1-2 phrases (~30 mots max). PAS un déroulé étape par étape à crochets. Jamais le nom d\'un concurrent.' },
