...
const ANALYZE_SCHEMA = {
  ...
  required: ['langueSource', 'sujet', 'viralite', 'hook', 'angle', 'structure', 'mecanismePsy', 'visuel', 'son', 'integrationProduit', 'format', 'cta', 'templateReutilisable', 'risques', 'scriptSource'],
};
...
function buildAdaptations({ analysis }) {
  const a = analysis;
  const text = `Vidéo virale analysée :\nSujet : ${a.sujet}\nFormule (template) : ${a.templateReutilisable}\nHook original : ${a.hook}\nRessort psychologique : ${a.mecanismePsy}\nComment un produit s'y intègre : ${a.integrationProduit}\n\nÉcris 3 adaptations FR pour promouvoir Creatikk sur cette formule. Chacune : astuce gratuite RÉELLE avant Creatikk, script verbatim complet, et une feature Creatikk DIFFÉRENTE d'une adaptation à l'autre.`;
  return { system: SYSTEM_ADAPT, content: [{ type: 'text', text }], schema: ADAPT_SCHEMA, maxTokens: 3500 };
}

module.exports = { callClaude, buildAnalyze, buildClassify, buildGenerate, buildConceptSheet, buildAdaptations };
