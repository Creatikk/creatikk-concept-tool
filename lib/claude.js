...
const CREATIKK = `Creatikk (creatikk.io) = outil IA de création de contenu court : génère des scripts viraux (30/60/90 s), monte des vidéos au Studio (clips de banque + voix off, ou génération IA), et analyse tes vidéos avant/après publication.`;
...
- Croise les 3 signaux : le SON (script/paroles + musique/tempo), le VISUEL (images), les STATS. Explique le mécanisme, pas juste un verdict.
...
Pour les adaptations Creatikk : chacune doit suivre CETTE structure, avec un scriptVerbatim complet mot pour mot. Si la vidéo source donne une astuce gratuite avant de citer son outil, tes adaptations DOIVENT faire pareil (astuce gratuite réelle → puis Creatikk).`;
...
const ANALYZE_SCHEMA = {
  ...
  properties: {
    ...
    adaptationsCreatikk: {
      type: 'array',
      description: '3 adaptations FR prêtes à filmer, suivant la structure obligatoire (valeur gratuite AVANT Creatikk)',
      items: {
        type: 'object',
        additionalProperties: false,
        properties: {
          ...
          astuceGratuite: { type: 'string', description: 'l\'astuce concrète, gratuite et actionnable donnée AVANT de mentionner Creatikk (le cœur de la méthode virale)' },
          scriptVerbatim: { type: 'string', description: 'le script COMPLET mot pour mot d\'une vidéo de 30-45s (~120-180 mots), langage parlé naturel, prêt prompteur : hook + astuce détaillée + pivot Creatikk + CTA. Aucune indication technique entre crochets.' },
  ...
  required: ['langueSource', 'sujet', 'viralite', 'hook', 'angle', 'structure', 'mecanismePsy', 'visuel', 'son', 'integrationProduit', 'format', 'cta', 'templateReutilisable', 'risques', 'scriptSource', 'adaptationsCreatikk'],
};

function buildAnalyze({ meta, transcript, framesB64 }) {
...
function buildGenerate({ niche, n, templates }) {
  const list = (templates || [])
    .map((t) => `- [${t.nom}] ${t.formule} — ressorts : ${t.ressorts || '?'} · force ${t.force} · niches : ${(t.niches || []).join(', ')}`)
    .join('\n');
  const text = `Niche : ${niche}\nNombre de concepts : ${n}\n\nTEMPLATES PROUVÉS (classés par force) :\n${list || '(mémoire vide — appuie-toi sur les grands ressorts viraux)'}\n\nGénère ${n} concepts distincts pour cette niche, chacun bâti sur un de ces templates (privilégie les plus forts et les plus adaptés à la niche).`;
...
module.exports = { callClaude, buildAnalyze, buildClassify, buildGenerate, buildConceptSheet };
